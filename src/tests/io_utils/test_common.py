...
from monkey_wrench.datetime_utils import Order, datetime_range
...
def test_pattern_exist(pattern, kwargs, res):
    assert res == io_utils.pattern_exists("This is a sample!", pattern, **kwargs)
...
def test_collect_files_in_dir(temp_dir, order, pattern):
    ...
    files = io_utils.collect_files_in_directory(temp_dir, order=order, pattern=pattern)
    ...
    if pattern == "non_existent_pattern":
        ...
        for file, dt in zip(files, datetime_objs, strict=True):
            assert f"{file.stem}.nc" == str(io_utils.seviri.input_filename_from_datetime(dt))
...
def test_copy_files_between_directories(temp_dir, pattern):
    ...
    io_utils.copy_files_between_directories(temp_dir, dest_directory, pattern=pattern)
    make_dummy_file(dest_directory / "excluded.ex")
    assert 4 == len(io_utils.collect_files_in_directory(dest_directory))
    assert 3 == len(io_utils.collect_files_in_directory(dest_directory, pattern=pattern))
...
def test_compare_files_against_reference(temp_dir):
    ...
    collected_files = io_utils.collect_files_in_directory(temp_dir)
    missing_files, corrupted_files = io_utils.compare_files_against_reference(
    ...
    missing_files, corrupted_files = io_utils.compare_files_against_reference(collected_files, number_of_processes=1)
...
@pytest.mark.parametrize(("func", "kwargs", "writer", "transform"), [
    (
            ...
            io_utils.write_items_to_txt_file_in_batches,
            lambda x: x
    ),
    (
            "query",
            OrderedDict(start_datetime=START_DATETIME, end_datetime=END_DATETIME),
            io_utils.write_items_to_txt_file,
            lambda x: list(x)),
])
def test_write_to_file_and_read_from_file(temp_dir, get_token_or_skip, func, kwargs, writer, transform):
    ...
    product_ids = set(io_utils.read_items_from_txt_file(temp_dir / Path("seviri_product_ids.txt")))
    assert expected_product_ids == product_ids


def test_create_datetime_dir(temp_dir):
    dir_path = io_utils.create_datetime_directory(datetime(2022, 3, 12), parent=temp_dir)
...
def make_dummy_datetime_files(datetime_objs: list[datetime], parent: Path):
    for datetime_obj in datetime_objs:
        dir_path = io_utils.create_datetime_directory(datetime_obj, parent=parent)
        filename = io_utils.seviri.input_filename_from_datetime(datetime_obj)
...
def test_temp_directory():
    ...
    with io_utils.temp_directory(".") as tmp:
